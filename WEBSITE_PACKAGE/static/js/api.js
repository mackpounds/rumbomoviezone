...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function createMovieDisplays(movies) {
        movies.forEach((movie, index) => {
            ...
            movieDisplay.querySelector("[data-movie-text]").textContent = title;
    ...
    fetch("https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=a3060ea251b922c2f7e4008e9c533905&page=7")
    .then(res => res.json())
    .then(data => {
        data.results.forEach(float => {
            const {poster_path, title} = float
    ...
    function showWrapMovies(movies){
        ...
        movies.forEach(movie => {
            ...
            wraptext.textContent = title

            wrapMovies.appendChild(wrapContent)
            // console.log(movie, wrapwallpaper.src)
        })
    ...
    //   TVS
    const secondColumnTemplate = document.querySelector(".second-column-template")
    const secondColumnContainer = document.querySelector("[data-scroll-two]")
    ...
    .then(data => {
        data.results.forEach(float => {
            ...
            const firstSlideTemplate = secondColumnTemplate.content.cloneNode(true).querySelector(".movie-container")
            const displayimg = firstSlideTemplate.querySelector(".movie-image-bar img")
            const displaytext = firstSlideTemplate.querySelector(".movie-name-bar span")
            displayimg.src = imagePath+poster_path
            displaytext.textContent = name

            // console.log(float, displaytext.textContent)

            secondColumnContainer.appendChild(firstSlideTemplate)
        })
    }).catch(error =>{ 
        if(TypeError){
            unknownErrorP.textContent = "(This error could be due to Failed attempt in connecting to the internet.)"
    ...
    // http://www.omdbapi.com/?i=tt3896198&apikey=62c5b883
    // http://www.omdbapi.com/apikey.aspx?VERIFYKEY=1cc052a3-0d2d-439d-8a81-5799f2ca2290
    // 62c5b883

    //   COMEDY SHOWS
    const thirdColumnTemplate = document.querySelector(".third-column-template")
    const thirdColumnContainer = document.querySelector("[data-scroll-three]")
    
    fetch("https://api.themoviedb.org/3/discover/tv?sort_by=popularity.desc&api_key=a3060ea251b922c2f7e4008e9c533905&page=11")
    .then(res => res.json())
    .then(data => {
        data.results.forEach(float => {
            const {poster_path, name} = float
            const firstSlideTemplate = thirdColumnTemplate.content.cloneNode(true).querySelector(".movie-container")
            const displayimg = firstSlideTemplate.querySelector(".movie-image-bar img")
            const displaytext = firstSlideTemplate.querySelector(".movie-name-bar span")
            displayimg.src = imagePath+poster_path
            displaytext.textContent = name
            ...
            thirdColumnContainer.appendChild(firstSlideTemplate)
        })
    }).catch(error => { 
        if(TypeError){
            unknownErrorP.textContent = "(This error could be due to Failed attempt in connecting to the internet.)"
    ...
    //   DRAMA
    const fourthColumnTemplate = document.querySelector(".fourth-column-template")
    const fourthColumnContainer = document.querySelector("[data-scroll-four]")
    
    fetch("https://api.themoviedb.org/3/discover/tv?sort_by=popularity.desc&api_key=a3060ea251b922c2f7e4008e9c533905&page=15")
    .then(res => res.json())
    .then(data => {
        data.results.forEach(float => {
            const {poster_path, name} = float
            const firstSlideTemplate = fourthColumnTemplate.content.cloneNode(true).querySelector(".movie-container")
            const displayimg = firstSlideTemplate.querySelector(".movie-image-bar img")
            const displaytext = firstSlideTemplate.querySelector(".movie-name-bar span")
            displayimg.src = imagePath+poster_path
            displaytext.textContent = name
            ...
            fourthColumnContainer.appendChild(firstSlideTemplate)
        })
    }).catch(error =>{ 
        if(TypeError){
            unknownErrorP.textContent = "(This error could be due to Failed attempt in connecting to the internet.)"
